# 订阅

v21

|||
| - |:-:|
|类别|10|
|全局规则|0|
|应用|70|
|应用规则|262|

## 规则类别

|||
| - |:-:|
|开屏广告|24|
|全屏广告|74|
|更新提示|27|
|权限申请|12|
|评价提示|5|
|青少年模式|2|
|局部广告|39|
|分段广告|28|
|功能类|46|
|其他|5|

## 全局规则



## 应用规则

||||
| - |:-:|-|
|Clousx6|1|<li>全屏广告-公告弹窗|
|Edge|4|<li>全屏广告<li>功能类-[关闭所有标签？]弹窗<li>功能类-关闭标签页<li>功能类-[前往CSDN APP阅读全文]弹窗|
|Fing|1|<li>其他-教程弹窗|
|Gmail|1|<li>分段广告-信息流广告|
|HMS Core|1|<li>全屏广告-弹窗广告|
|QQ|26|<li>开屏广告<li>局部广告-消息页面顶部横幅广告<li>分段广告-好友动态-卡片广告<li>全屏广告-弹窗广告<li>局部广告-顶部SVIP广告<li>分段广告-动态页[好友热播/为你推荐]<li>功能类-扫一扫登录确认<li>局部广告-浮窗广告<li>功能类-自动勾选原图<li>功能类-自动查看原图<li>分段广告-QQ小世界评论区广告<li>开屏广告-QQ小程序开屏广告<li>更新提示<li>更新提示-消息页面-顶部<li>分段广告-钱包页卡片广告<li>全屏广告-首页弹窗广告<li>分段广告-天气页卡片广告<li>全屏广告-新春回馈礼弹窗<li>局部广告-聊天页面广告<li>全屏广告-个人主页广告<li>全屏广告-腾讯文档页面-"使用App"弹窗<li>功能类-自动抢红包<li>全屏广告-账号信息页面-新人SVIP领取弹窗<li>功能类-申请入群后自动点击右上角关闭<li>局部广告-动态页-卡片广告<li>功能类-授权登录时关闭获取QQ好友关系|
|TapTap|3|<li>分段广告-游戏浏览页面推荐广告<li>更新提示<li>青少年模式-首页顶部横幅提示|
|TIM|2|<li>功能类-自动点击查看原图<li>功能类-发送图片时自动勾选[原图]|
|畅听FM|1|<li>全屏广告-弹窗广告|
|纯纯打码|1|<li>全屏广告-解锁高级功能|
|得物|1|<li>其他-截屏分享|
|饿了么|9|<li>更新提示<li>全屏广告-红包弹窗<li>全屏广告-活动弹窗<li>全屏广告-付款后弹窗广告<li>全屏广告-吃货卡续费弹窗<li>局部广告-首页底部浮窗广告<li>权限申请<li>全屏广告-获取订单信息通知<li>评价提示-底部调研邀请卡片|
|高佣联盟|1|<li>更新提示|
|哈啰|5|<li>更新提示<li>全屏广告-购卡弹窗<li>全屏广告<li>全屏广告-弹窗广告<li>局部广告-右侧悬浮广告|
|华图在线|2|<li>全屏广告-首页弹窗广告<li>全屏广告|
|华为浏览器|3|<li>开屏广告<li>全屏广告<li>局部广告-卡片广告|
|华为钱包|3|<li>局部广告-钱包扩展服务提示<li>全屏广告-小额免密支付弹窗<li>分段广告-首页卡片广告|
|华为视频|1|<li>开屏广告|
|华为天气|3|<li>开屏广告<li>局部广告-卡片广告<li>分段广告-卡片广告|
|华为音乐|3|<li>开屏广告<li>全屏广告-VIP弹窗广告<li>局部广告-推荐卡片广告|
|华为音乐|4|<li>开屏广告<li>局部广告-播放界面直播浮窗<li>全屏广告-弹窗广告<li>局部广告-卡片广告|
|华为应用市场|4|<li>开屏广告<li>更新提示<li>全屏广告-热门应用推荐弹窗<li>局部广告-卡片广告|
|华为阅读|1|<li>开屏广告|
|华为运动健康|7|<li>开屏广告<li>局部广告-“我的”页会员广告<li>全屏广告-弹窗广告<li>功能类-请求开启个性化推荐弹窗<li>局部广告-底部横幅广告<li>功能类-请求开启[实时推送步数]弹窗<li>更新提示|
|华为主题|5|<li>开屏广告<li>局部广告-悬浮广告<li>分段广告-卡片广告<li>全屏广告-首页活动弹窗<li>全屏广告|
|交管12123|3|<li>全屏广告<li>功能类-确认退出<li>更新提示|
|今日校园|1|<li>更新提示|
|金山文档|3|<li>局部广告-列表卡片广告<li>更新提示<li>全屏广告|
|酷安|5|<li>开屏广告<li>分段广告-信息流广告<li>更新提示<li>全屏广告<li>局部广告-[大家还下载了]应用推荐|
|美团|10|<li>更新提示<li>全屏广告-弹窗广告<li>评价提示-订单调查弹窗<li>局部广告-悬浮广告<li>权限申请<li>全屏广告<li>全屏广告-新人返场特惠<li>全屏广告-视频页弹窗广告<li>功能类-付款后自动点击完成<li>局部广告-直播讲解悬浮窗|
|蜜雪冰城鸿蒙版|1|<li>全屏广告-首页弹窗广告|
|蜻蜓FM|1|<li>局部广告-播放界面的广告|
|权限控制器|2|<li>权限申请-相机允许本次<li>权限申请-位置允许本次|
|软件包安装程序|1|<li>功能类-自动安装应用|
|软件包安装程序|2|<li>功能类-自动安装/更新软件<li>功能类-安全警告弹窗|
|顺丰速运|4|<li>开屏广告<li>更新提示<li>其他-专属月报弹窗<li>全屏广告-弹窗广告|
|搜索|1|<li>局部广告-卡片广告|
|淘宝|14|<li>开屏广告<li>全屏广告-弹窗广告<li>局部广告-消息页面热门活动卡片<li>局部广告-悬浮广告<li>全屏广告<li>功能类-各级页面添加到首页弹窗<li>更新提示<li>功能类-关闭小额免密支付<li>功能类-将小组件添加到手机桌面<li>权限申请-悬浮窗权限<li>全屏广告-花呗升级报送征信<li>全屏广告-抢天降补贴弹窗<li>全屏广告-[「0元下单」权益]弹窗<li>局部广告-商品详情页直播悬浮窗|
|腾讯会议|1|<li>更新提示|
|腾讯文档|4|<li>开屏广告<li>更新提示<li>局部广告-卡片广告<li>全屏广告-弹窗广告|
|天猫精灵|2|<li>更新提示<li>全屏广告-超级会员弹窗|
|铁路12306|1|<li>开屏广告|
|完美校园|2|<li>更新提示<li>其他-密码过期提示弹窗|
|网易云音乐|12|<li>开屏广告<li>分段广告-卡片广告<li>局部广告-卡片广告<li>全屏广告-弹窗广告<li>全屏广告-VIP弹窗<li>更新提示<li>局部广告-播放界面广告<li>局部广告-发现页顶部视频广告<li>分段广告-评论区广告<li>功能类-扫码后自动点击[授权登录]<li>全屏广告-[获得新徽章]弹窗<li>分段广告-搜索页广告|
|微博极速版|2|<li>分段广告-信息流广告<li>评价提示-APP评分弹窗|
|微博轻享版|2|<li>开屏广告<li>分段广告-信息流广告|
|微信|24|<li>分段广告-朋友圈广告<li>功能类-电脑微信快捷自动登录<li>功能类-自动授权登录<li>功能类-微信读书网页版扫码登录自动授权<li>功能类-微信红包自动领取<li>功能类-自动选中发送原图<li>功能类-自动查看原图<li>开屏广告-微信小程序<li>功能类-网页版文件传输助手扫码自动授权<li>青少年模式<li>功能类-青少年模式自动点击验证密码<li>功能类-订阅号-展开更早的消息<li>功能类-开启青少年模式后的每日验证<li>局部广告-订阅号消息-推荐文章<li>功能类-付款后自动点击完成/返回商家<li>分段广告-订阅号消息-视频推荐<li>功能类-解锁 Windows 微信<li>全屏广告-文档页面-腾讯文档APP弹窗<li>分段广告-订阅号消息-推荐阅读<li>权限申请-权限申请弹窗<li>功能类-付款时自动点击[支付]<li>分段广告-订阅号消息内容-广告<li>功能类-自动点击[查看原视频]<li>全屏广告-小程序弹窗广告|
|系统更新|1|<li>更新提示-智能更新弹窗|
|下载管理|1|<li>更新提示|
|雪球|2|<li>局部广告-信息流广告<li>局部广告-评论区广告|
|雪球基金|1|<li>全屏广告-基金页面弹窗广告|
|迅游手游加速器|2|<li>开屏广告<li>分段广告-卡片广告|
|应用安装器|1|<li>功能类-自动安装应用|
|云班课|1|<li>全屏广告-首页班课列表广告|
|云空间|1|<li>全屏广告-[升级云空间]弹窗|
|云闪付|7|<li>分段广告-支付完成界面卡片广告<li>全屏广告-付款后信用卡广告<li>局部广告-右侧悬浮广告<li>权限申请-位置<li>全屏广告<li>全屏广告-天天签到-获得新勋章提示<li>全屏广告-领取优惠弹窗|
|支付宝|17|<li>全屏广告-关闭花呗升级弹窗<li>权限申请<li>全屏广告<li>更新提示-版本更新弹窗<li>功能类-设置支付宝小组件<li>全屏广告-借呗消费信贷协议<li>功能类-支付后自动点击完成<li>全屏广告-弹窗广告<li>全屏广告-"下次支付用花呗"弹窗<li>分段广告-扫码乘车后出现的广告<li>功能类-关闭免密支付开关<li>分段广告-服务消息页面-卡片广告<li>全屏广告-[0元下单权益]弹窗<li>局部广告-卡片广告<li>其他-蚂蚁理财社区-[加入同路人]弹窗<li>全屏广告-[银行卡还款服务升级]弹窗<li>开屏广告-小程序开屏广告|
|智慧搜索|1|<li>分段广告-下拉搜索横幅广告|
|中国农业银行|4|<li>开屏广告<li>更新提示<li>权限申请<li>全屏广告|
|中国移动|8|<li>开屏广告<li>更新提示<li>全屏广告-首页弹窗广告<li>全屏广告<li>权限申请-请求获取剪贴板权限弹窗<li>局部广告-右侧悬浮小图标<li>评价提示-请求好评弹窗<li>权限申请-通讯录权限申请弹窗|
|中信银行|4|<li>更新提示<li>全屏广告-弹窗广告<li>全屏广告<li>局部广告-理财产品悬浮广告|
|Android 系统文件选择器|1|<li>功能类-授权第三方应用访问文件夹|
|e充电|1|<li>全屏广告-首页弹窗广告|
|ES文件浏览器|2|<li>全屏广告-弹窗广告<li>局部广告|
|Google Play 服务|1|<li>功能类-谷歌填充密码自动授权|
|Google Play 商店|2|<li>功能类-Play保护机制弹窗<li>全屏广告-弹窗广告|
|Google翻译|1|<li>功能类-关闭登录备份翻译记录弹窗|
|PU口袋校园|4|<li>开屏广告<li>全屏广告-弹窗广告<li>局部广告-首页-右侧浮窗广告<li>更新提示|
|QQ音乐flyme版|1|<li>权限申请-存储权限弹窗|
|X(推特)|7|<li>分段广告-主页信息流广告<li>分段广告-帖子详情页、搜索页信息流广告<li>分段广告-用户资料页信息流广告<li>评价提示<li>全屏广告<li>功能类-自动点击翻译<li>功能类-自动点击"显示更多帖子"|
